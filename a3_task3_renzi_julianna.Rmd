---
title: "PhD vs. M.S. advisor's most recent publication: A text analysis"
...
ezzat_text <- pdf_text("Ezzat_et_al_2020.pdf")
```
...
```{r, message = FALSE, warning = FALSE}
he_text <- pdf_text("He_et_al_2020.pdf")
```
...
Again starting with the afinn lexicon that ranks words as positive or negative from a scale of -5 to 5
```{r, message = FALSE, warning = FALSE}
afinn_lex <- get_sentiments("afinn")
...
```{r}
(nrc_Sill / nrc_Der) + plot_annotation(
  title = "NRC Sentiment analysis"
) + 
  plot_layout(widths = c(4, 4), heights = c(26, 26))
```
